...
A hands-on implementation demonstrating how to securely deliver static web assets using Amazon CloudFront and an Amazon S3 origin, enforcing least-privilege edge security. By implementing **Origin Access Control (OAC)** with Signature Version 4 (SigV4) request signing, origin traffic is strictly locked down so only requests originating from the authorized CloudFront distribution are permitted, while direct public traffic to the S3 bucket endpoint is rejected.
...
1. **Storage Origin**: **Amazon S3** stores the static web assets (`index.html` and media files) with access governed by an explicit resource policy.
...
### Stage 1: Create CloudFront Distribution & Configure OAC
...
### Stage 2: Enforce Least-Privilege S3 Bucket Policy
...
### Stage 3: Configure Distribution Settings & Default Root Object
...
### Stage 4: Create CloudFront Cache Invalidation
...
### Stage 5: Verify Edge Delivery via HTTPS
...
### Stage 6: Security Verification (Direct S3 Access Blocked)
Tested origin isolation by attempting direct access to the website endpoint URL of the S3 bucket. Because OAC is actively enforced by the bucket policy, any direct client requests bypassing CloudFront return a `403 Forbidden` error. This confirms that only requests signed by the authorized CloudFront distribution can access the content.
